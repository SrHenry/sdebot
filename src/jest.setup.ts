function normalize(
  ...
  if (typeof Buffer !== 'undefined' && Buffer.isBuffer(obj)) {
    return `[Buffer len:${obj.length} base64:${obj
      .toString('base64')
      .slice(0, 32)}…]`;
  }
  ...
  if (ArrayBuffer.isView(obj)) {
    ...
    if ('length' in obj && typeof (obj as any).length === 'number') {
      return `[TypedArray ${name} len:${(obj as any).length} byteLength:${
        obj.byteLength
      }]`;
    }
  ...
  if (obj instanceof Map) {
    ...
    for (const [k, v] of obj.entries()) {
      ...
      mapObj[`[MapKey:${keyStr}]`] = normalize(
        v,
        seen,
        `${path}[MapKey:${keyStr}]`,
      );
  ...
  if (obj instanceof Set) {
    const arr = Array.from(obj).map((v, i) =>
      normalize(v, seen, `${path}[${i}]`),
    );
    return arr;
  }
